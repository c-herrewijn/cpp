...
void ClapTrap::attack(const std::string &target)
{
    if (this->_hitPoints <= 0) {
        std::cout << "ClapTrap " << this->_name << " can't attack because it is dead!"
                  << std::endl;
    }
...
void ClapTrap::beRepaired(unsigned int amount)
{
    if (this->_hitPoints <= 0) {
        ...
        std::cout << "ClapTrap " << this->_name << " repairs itself and heals "
...
{
    if (this->_hitPoints <= 0) {
        std::cout << "ClapTrap " << this->_name << " is hit for " << amount
                  << " damage, but is already dead!" << std::endl;
    }
    else {
        std::cout << "ClapTrap " << this->_name << " is hit and takes " << amount
